...
func (s *GPG2Signer) SignFiles(inputs []signer.NamedUnsignedFile, options interface{}) (signedFiles []signer.NamedSignedFile, err error) {
	...
	if err != nil {
		err = fmt.Errorf("gpg2: error creating tempdir for signing: %w", err)
		return
	}
	defer func() {
	...
	for i, input := range inputs {
		...
		if err != nil {
			return nil, fmt.Errorf("gpg2: failed to write tempfile %d for signing: %w", i, err)
		}
	...
	if err != nil {
		return nil, fmt.Errorf("gpg2: failed to create passphrase pipe: %w", err)
	}
	...
	if _, err = io.WriteString(passphraseWriter, passphrases); err != nil {
		...
		return nil, fmt.Errorf("gpg2: failed to write passphrase to pipe: %w", err)
	}
	passphraseWriter.Close()
