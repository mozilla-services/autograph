...
// makeMARSignableBlock makes a signable block by reading everything except the signature entries.
// The beginning of the MAR until right after the signature entry headers are covered,
// then the signatures are skipped, then the additional sections until the end of the file are covered.
func makeMARSignableBlock(input []byte) (signableBlock []byte, err error) {
	var (
		cursor         int
		sigHeader      SignaturesHeader
		sigEntryHeader SignatureEntryHeader
	)
	if len(input) < HeaderLen+SignaturesHeaderLen+AdditionalSectionsHeaderLen+IndexHeaderLen {
		err = fmt.Errorf("input is smaller than minimum MAR size and cannot be parsed")
		return
	}

	// First copy the main header and signature header
	signableBlock = append(signableBlock, input[:HeaderLen+SignaturesHeaderLen]...)

	// parse the signature entries to copy the header of each entry but skip existing signature bytes
	cursor += HeaderLen
	err = parse(input, &sigHeader, cursor, SignaturesHeaderLen)
	if err != nil {
		return
	}
	cursor += SignaturesHeaderLen
	for i = 0; i < sigHeader.NumSignatures; i++ {
		// add the signature header of each entry to the signed block
		signableBlock = append(signableBlock, input[cursor:cursor+SignatureEntryHeaderLen]...)
		err = parse(input, &sigEntryHeader, cursor, SignatureEntryHeaderLen)
		if err != nil {
			return
		}
		// skip the signature itself the next signing entry
		cursor += SignatureEntryHeaderLen + sigEntryHeader.Size
	}
	// the rest of the input is copied to the signable block
	signableBlock = append(signableBlock, input[cursor:]...)
	return
}

func parse(input []byte, data interface{}, cursor, readlen int) error {
	if len(input) < cursor+readlen {
		return fmt.Errorf("requested read overruns input buffer")
	}
	return binary.Read(input[cursor:cursor+readlen], binary.BigEndian, data)
}
