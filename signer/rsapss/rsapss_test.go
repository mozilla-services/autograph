...
func TestSignHash(t *testing.T) {
	...
	t.Run("Verifies", func(t *testing.T) {
		...
		err := VerifySignature(pubKey.(*rsa.PublicKey), digest, sig.(*Signature).Data, rsa.PSSOptions{
...
func TestSignDataWithSha256(t *testing.T) {
	input := []byte("this is the sha256 input")
	shasum := sha256.Sum256(input)
	digest := shasum[:]

	// initialize a signer
	s := assertNewSignerWithConfOK(t, rsapsssha256signerconf)
...
func TestVerifySignatureFromB64(t *testing.T) {
	...
	t.Run("verifies valid input", func(t *testing.T) {
		...
		pubKey := rsaKey.Public()
		err := VerifySignature(pubKey.(*rsa.PublicKey), _digest, _sig.(*Signature).Data, rsa.PSSOptions{
			SaltLength: _s.rsaPssSaltLength,
			Hash:       _s.rsaPssHash,
		})
		if err != nil {
			t.Fatalf("failed to verify signature: %v", err)
		...
		err = VerifySignatureFromB64(b64Digest, b64Sig, b64PubKey, rsa.PSSOptions{
			SaltLength: _s.rsaPssSaltLength,
			Hash:       _s.rsaPssHash,
		})
		if err != nil {
	...
	t.Run("fails for invalid b64 input", func(t *testing.T) {
		...
		err := VerifySignatureFromB64("aieeee", b64Sig, b64PubKey, rsa.PSSOptions{
			SaltLength: _s.rsaPssSaltLength,
			Hash:       _s.rsaPssHash,
		})
		if err == nil {
	...
	t.Run("fails for invalid b64 sig", func(t *testing.T) {
		...
		err := VerifySignatureFromB64(b64Digest, "aieeee", b64PubKey, rsa.PSSOptions{
			SaltLength: _s.rsaPssSaltLength,
			Hash:       _s.rsaPssHash,
		})
		if err == nil {
	...
	t.Run("fails for invalid b64 pubkey", func(t *testing.T) {
		...
		err := VerifySignatureFromB64(b64Digest, b64Sig, "aieeee", rsa.PSSOptions{
			SaltLength: _s.rsaPssSaltLength,
			Hash:       _s.rsaPssHash,
		})
		if err == nil {
	...
	t.Run("fails for invalid pubkey pem", func(t *testing.T) {
		...
		err := VerifySignatureFromB64(b64Digest, b64Sig, "", rsa.PSSOptions{
			SaltLength: _s.rsaPssSaltLength,
			Hash:       _s.rsaPssHash,
		})
		if err == nil {
	...
	t.Run("fails for invalid pubkey type", func(t *testing.T) {
		...
		err := VerifySignatureFromB64(b64Digest, b64Sig, base64.StdEncoding.EncodeToString([]byte(nonRSAPrivateKey)), rsa.PSSOptions{
			SaltLength: _s.rsaPssSaltLength,
			Hash:       _s.rsaPssHash,
		})
		if err == nil {
...
var rsapsssignerconf = signer.Configuration{
	ID:                         "rsapsstest",
	Type:                       Type,
	RSAPSSSaltLengthEqualsHash: true,
	RSAPSSHash:                 "sha1",
...
var rsapsssha256signerconf = signer.Configuration{
	ID:                         "rsapsstestsha256",
	Type:                       Type,
	RSAPSSSaltLengthEqualsHash: false,
	RSAPSSHash:                 "sha256",
	PrivateKey: `-----BEGIN RSA PRIVATE KEY-----
MIIEpAIBAAKCAQEAtEM/Vdfd4Vl9wmeVdCYuWYnQl0Zc9RW5hLE4hFA+c277qanE
8XCK+ap/c5so87XngLLfacB3zZhGxIOut/4SlEBOAUmVNCfnTO+YkRk3A8OyJ4XN
