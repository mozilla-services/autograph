-- test signer config for soft-hsm
INSERT INTO signer(id, type, mode, public, comments)
VALUES('hsm-testmar', 'mar', null, jsonb_build_object('privatekey', 'testrsa2048'), null),
('hsm-testmar2', 'mar', null, jsonb_build_object('privatekey', 'testrsa4096'), null),
('hsm-testmarecdsa', 'mar', null, jsonb_build_object('privatekey', 'testecdsap256'), null),
('hsm-testcsp384', 'contentsignature', null, jsonb_build_object('privatekey', 'testecdsap384'), null),
('hsm-webextensions-rsa', 'xpi', 'add-on', jsonb_build_object(
  'rsacacheconfig', jsonb_build_object(
    'numkeys', 5,
    'numgenerators', 1,
    'generatorsleepduration', '1m',
    'fetchtimeout', '100ms',
    'statssamplerate', '1m'
  ), 'privatekey', 'webextrsa4096',
  'certificate', '-----BEGIN CERTIFICATE-----

E5bRcttRlhEdN4NJ2vWJnOH0DENHy6TEwACINJVx6ftucfPfvOxI
-----END CERTIFICATE-----
'), null),
('hsm-extensions-ecdsa', 'xpi', 'extension', jsonb_build_object(
  'privatekey', 'ext-ecdsa-p384',
  'certificate', '-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
'), null),
('hsm-normandy', 'contentsignaturepki', null, jsonb_build_object(
  'validity', '708h',
  'clockskewtolerance', '10m',
  'chainlocation', '/tmp/chains/',
  'x5u', 'file:///tmp/chains/',
  'issuerprivkey', '${NORMANDY_IS_PRIV_KEY}',
  'issuercert', '${NORMANDY_IS_CERT}',
  'cacert', '${NORMANDY_CA_CERT}'), null),
('hsm-kinto', 'contentsignaturepki', null, jsonb_build_object(
  'validity', '708h',
  'clockskewtolerance', '10m',
  'chainlocation', '/tmp/chains/',
  'x5u', 'file:///tmp/chains/',
  'issuerprivkey', '${KINTO_IS_PRIV_KEY}',
  'issuercert', '${KINTO_IS_CERT}',
  'cacert', '${KINTO_CA_CERT}'), null),
('hsm-extensions-ecdsa-expired-chain', 'xpi', 'extension', jsonb_build_object(
  'privatekey', 'ext-ecdsa-p384-2',
  'certificate', '-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
'), null)
ON CONFLICT(id) DO UPDATE
SET public = EXCLUDED.public;

INSERT INTO auth_signers(auth, signer)
VALUES('alice', 'hsm-testmar'),
('alice', 'hsm-testmar2'),
('alice', 'hsm-testmarecdsa'),
('alice', 'hsm-testcsp384'),
('alice', 'hsm-webextensions-rsa'),
('alice', 'hsm-extensions-ecdsa'),
('alice', 'hsm-normandy'),
('alice', 'hsm-kinto'),
('alice', 'hsm-extensions-ecdsa-expired-chain')
ON CONFLICT DO NOTHING;
